...
export type DialogContentSize = "sm" | "md" | "lg" | "xl" | "2xl" | "6xl" | "full" | "max";
...
export interface DialogButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: "primary" | "secondary" | "warning" | "danger" | "ghost";
  size?: "sm" | "md";
  loading?: boolean;
}

export interface DialogFieldProps extends React.HTMLAttributes<HTMLDivElement> {
  label?: string;
  smallLabel?: boolean;
  required?: boolean;
}

export interface DialogAlertProps extends React.HTMLAttributes<HTMLDivElement> {
...
export const dialogInputClass = 
  "w-full bg-black/20 border border-white/10 rounded-md py-2.5 px-4 text-sm text-white placeholder:text-textMuted/40 hover:border-white/20 focus:border-white/30 outline-none transition-colors";

export const dialogSelectClass = 
  "w-full bg-black/20 border border-white/10 rounded-md py-2.5 px-4 text-sm text-white hover:border-white/20 focus:border-white/30 outline-none transition-colors appearance-none cursor-pointer";
...
>(({ className, size = "md", hideCloseButton = false, overlayClassName, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 w-full -translate-x-1/2 -translate-y-1/2 bg-surface border border-white/10 rounded-xl shadow-2xl flex flex-col max-h-[90vh]",
  ...
  ({ className, withBg = false, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        withBg && "bg-white/5 border-b border-white/10",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn("text-lg font-bold text-white font-exo2 uppercase tracking-wide", className)}
    {...props}
  />
));
...
const DialogBody = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex-1 overflow-y-auto p-6 pt-2 custom-scrollbar", className)}
      {...props}
    />
  )
);
...
const DialogFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex justify-end gap-3 p-6 pt-4 border-t border-white/5", className)}
      {...props}
    />
  )
);
...
const DialogField = React.forwardRef<HTMLDivElement, DialogFieldProps>(
  ({ className, label, smallLabel = false, required = false, children, ...props }, ref) => (
    <div ref={ref} className={cn("space-y-2", className)} {...props}>
      {label && (
        <label className={cn(
          smallLabel ? "block text-[10px] font-bold text-textMuted uppercase tracking-wider" : "block text-sm font-medium text-textMuted"
        )}>
          {label}
...
const DialogAlert = React.forwardRef<HTMLDivElement, DialogAlertProps>(
  ({ className, variant = "info", icon, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex gap-3 p-4 rounded-lg border", variantClasses[variant], className)}
        {...props}
      >
